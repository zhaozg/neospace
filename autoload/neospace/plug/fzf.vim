...
function neospace#plug#fzf#Session()
  call fzf#run({
...
" Configuration Files
" ------------------------------------------------------------------
function neospace#plug#fzf#Open()
  let l:open = [
        \ '~/.neospace',
        \ '~/.neospace/init.vim',
        \ '~/.bashrc',
...
" Runtimepath
" ------------------------------------------------------------------
function neospace#plug#fzf#Rtp()
...
" Oldfiles
" ------------------------------------------------------------------
function neospace#plug#fzf#Oldfiles()
  redir => out
  silent oldfiles
  redir END
...
" FZF commands
" ------------------------------------------------------------------
function neospace#plug#fzf#FZFCmd()
  redir => cout
  silent command
  redir END
  let list = split(cout, "\n")
...
" ------------------------------------------------------------------
" neospace related functions
" ------------------------------------------------------------------
function neospace#plug#fzf#Func()
  redir => fout
  silent function
  redir END
  let list = split(fout, "\n")
  let list = filter(list, 'stridx(v:val, "neospace#") > -1')
...
" FZF find file
" ------------------------------------------------------------------
function neospace#plug#fzf#FindFileInProject()
  exe ':FZF '.neospace#util#RootDirectory()
endfunction
...
command! -nargs=* Rag
  \ call fzf#vim#ag(<q-args>, extend({
    \ 'dir': neospace#util#RootDirectory(),
    \ 'options': '--prompt="'.neospace#util#RootDirectory().'> "'},
    \ g:fzf_layout
    \))
function neospace#plug#fzf#SearchInProject()
  exe ':Rag'
endfunction
...
" ------------------------------------------------------------------
function neospace#plug#fzf#SearchCword()
  call fzf#vim#ag(
        \ expand('<cword>'),{
        \ 'dir': neospace#util#RootDirectory(),
...
function neospace#plug#fzf#SearchBcword()
...
function neospace#plug#fzf#Jumps(...)
  redir => cout
  silent jumps
  redir END
  let list = split(cout, "\n")
